...
#define SPOTLIGHT 2
#define MAX_LIGHTS 5 // added maximum lights supported
...
{
int type;
// removed ambient here
vec3 color;
vec4 position;
vec3 direction;
float cutoff;
float exponent;
} lights[MAX_LIGHTS];// use an array of lights (defined by MAX_LIGHTS)
...
uniform int light_count; // the actual numbers of lights used
uniform vec3 ambient_color; // ambient is now a global value (not stored in lights)
